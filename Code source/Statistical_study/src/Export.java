...
public class Export {
    ...
    private void exportPDF() {
        ...
        try {
         ...
         // Ajouter le tableau de mesures depuis le JTable de l'interface
         writeSection("Measurement table");
         JTable measurementTable = createMeasurementTableFromJTable();
         writeTableWithLinesPaginated(measurementTable, document);
            ...
            writeTableWithLinesPaginated(main.table2, document);
      ...
      content.newLineAtOffset(MARGIN + 10, y);
      content.setNonStrokingColor(Color.RED);  // <-- texte en rouge
      content.showText(main.lblNewLabel_7.getText());
      content.endText();
      content.setNonStrokingColor(Color.BLACK); // <-- remet noir pour le reste
      y -= 14;
   ...
   writeTableWithLinesPaginated(document, chiTable);
           ...
           // Ajouter la section Validation à la fin
    ...
    // TABLEAU AVEC LIGNES
    // ==========================
    private void writeTableWithLinesPaginated(JTable table, PDDocument document) throws IOException {
        ...
        for (int r = 0; r < table.getRowCount(); r++) {
            ...
            if (currentY < BOTTOM_MARGIN + rowHeight) {
                content.close();
                content = createNewPage(document);
            ...
            // Dessiner l'entête sur chaque nouvelle page
            if (drawHeader) {
                // fond gris
        ...
        y = currentY - 10; // mettre à jour y pour le contenu suivant
    }
    ...
    private void drawHeader(PDDocument document) throws IOException {
        try {
            PDImageXObject logo = PDImageXObject.createFromFile("icons/logo.png", document);
        ...
        writeLineAtColor("Printed on: " + d.format(new Date()), 450, 735, font, 9, Color.RED);
        writeLineAtColor("Time: " + t.format(new Date()), 450, 720, font, 9, Color.RED);
 ...
 // ==========================
    ...
    private void writeValidationSection() throws IOException {
        // Dessiner une ligne horizontale
        content.setStrokingColor(Color.BLUE);  // couleur de la ligne
        ...
        content.newLineAtOffset(MARGIN, y);
        content.showText("Validation");
        content.endText();
        y -= 30;

        // Labels Operator / Validated by
        float columnGap = 300; // écart entre deux colonnes
        content.setFont(font, 10);

        // Ligne 1 : Operator / Validated by
        content.beginText();
        content.newLineAtOffset(MARGIN, y);
        content.showText("Operator :");
        content.endText();

        content.beginText();
        content.newLineAtOffset(MARGIN + columnGap, y);
        content.showText("Validated by:");
        content.endText();
        y -= 30;

        // Ligne 2 : Signature / Signature
        content.beginText();
        content.newLineAtOffset(MARGIN, y);
        ...
        content.showText("Signature :");
        content.endText();
        y -= 50; // laisser un peu d’espace
    }

    private JTable createMeasurementTableFromJTable() {
        if (main.table.getRowCount() == 0) return new JTable();

        ArrayList<Double> valuesN = new ArrayList<>();
        // Lecture de la deuxième colonne (index 1) de main.table
        for (int r = 0; r < main.table.getRowCount(); r++) {
            Object val = main.table.getValueAt(r, 1); // colonne N
            if (val != null) {
                try {
                    double d = Double.parseDouble(val.toString().replace(",", "."));
                    valuesN.add(d);
                } catch (NumberFormatException ignored) {}
            }
        }

        if (valuesN.isEmpty()) return new JTable();

        // Déterminer le nombre de colonnes : max 10 par ligne
        int cols = 10;
        int rows = (int) Math.ceil(valuesN.size() / (double) cols);

        Object[][] data = new Object[rows][cols];
        int index = 0;
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                if (index < valuesN.size()) {
                    data[r][c] = valuesN.get(index++);
                } else {
                    data[r][c] = ""; // cellule vide
                }
            }
        }
        ...
        for (int i = 0; i < cols; i++) columnNames[i] = "N" + (i+1);
